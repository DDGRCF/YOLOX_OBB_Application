...
template <typename T>
T single_box_iou_rotated(const T *const box1_raw, const T *const box2_raw) {
    Point<T> pts1[4];
    Point<T> pts2[4];
    T area1{0}; T area2{0};
    ...
    get_rotated_vertices<T>(box1_raw, pts1, center_shift_x, center_shift_y);
    get_rotated_vertices<T>(box2_raw, pts2, center_shift_x, center_shift_y);

    if (area1 < 1e-6 || area2 < 1e-6) {
        return 0.f;
    }

    T intersection = rotated_boxes_intersection<T>(pts1, pts2);
